...
describe('HTMLProcessor.applyToElement', () => {
    ...
    for (const test of [
        ...
        it(test.in, () => {
            ...
            if (test.browserOnly && !isBrowser)
                return;
            const result = apply(test.in, wbr);
            const isEqual = isEqualNodeWithStyleNormalized(parseFromString(result).body, parseFromString(out).body);
            if (!isEqual) {
...
describe('HTMLProcessor.applyToElement.separator.node', () => {
    it('should clone separator element deeply', () => {
        ...
        const isEqual = isEqualNodeWithStyleNormalized(doc.body, parseFromString(expected).body);
        if (!isEqual) {
